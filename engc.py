...
# ==========================================
# 1. HUMANIZED SYSTEM PROMPT (इंसानी टीचर का दिमाग)
# ==========================================
SYSTEM_PROMPT = """
...
5. CLEAN SUMMARY FORMAT (Keep it brief, human, and conversational):
...
st.title("👩‍🏫 CBSE AI Teaching Assistant (Powered by Groq)")
...
with st.sidebar:
    ...
    api_key_input = st.text_input("Groq API Key 🔑", type="password", key="sidebar_key")
    # आपकी API Key यहाँ पहले से सेट है
    DEFAULT_API_KEY = "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" 
    API_KEY = api_key_input if api_key_input else DEFAULT_API_KEY
    
    # बिल्कुल सही और नया Qwen विज़न मॉडल ID
...
with col1:
    st.markdown("##### 1. Question Paper (Optional)")
...
with col2:
    st.markdown("##### 2. Answer Key (Optional)")
...
with col3:
    st.markdown("##### 3. Student's Copy (Required)")
...
if len(student_images_raw) > 0 and not st.session_state.messages:
    if st.button("🚀 Evaluate Copy Now", type="primary", key="eval_btn"):
        with st.spinner("Analyzing handwriting with Qwen Vision... Please wait."):
